...
module sa2x2_controller #(
    ...
    output logic signed [WORD_W-1:0]  read_data,
    output logic                      busy
);
    ...
    localparam logic [4:0]               WORD_W_U = 5'(WORD_W);
    ...
    logic signed [WORD_W-1:0] bank_a_q [0:3];
    logic signed [WORD_W-1:0] bank_b_q [0:3];
    logic signed [WORD_W-1:0] bank_c_q [0:3];
    ...
    logic signed [WORD_W-1:0] array_c00;
    logic signed [WORD_W-1:0] array_c01;
    logic signed [WORD_W-1:0] array_c10;
    logic signed [WORD_W-1:0] array_c11;

    integer word_idx;

    function automatic logic signed [WORD_W-1:0] relu_word(
        input logic signed [WORD_W-1:0] value
    );
        begin
            relu_word = value[WORD_W-1] ? '0 : value;
        end
    endfunction

    function automatic logic signed [WORD_W-1:0] shift_word(
        input logic signed [WORD_W-1:0] value,
        input logic [3:0]               amount
    );
        begin
            if ({1'b0, amount} >= WORD_W_U)
                shift_word = value[WORD_W-1] ? {WORD_W{1'b1}} : '0;
            else
    ...
    always_comb begin
        ...
        case (state_q)
            ...
            STATE_FEED0: begin
                array_a_left0 = bank_a_q[0][DATA_W-1:0];
                array_b_top0 = bank_b_q[0][DATA_W-1:0];
            end
            STATE_FEED1: begin
                array_a_left0 = bank_a_q[1][DATA_W-1:0];
                array_a_left1 = bank_a_q[2][DATA_W-1:0];
                array_b_top0 = bank_b_q[2][DATA_W-1:0];
                array_b_top1 = bank_b_q[1][DATA_W-1:0];
            end
            STATE_FLUSH0: begin
                array_a_left1 = bank_a_q[3][DATA_W-1:0];
                array_b_top1 = bank_b_q[3][DATA_W-1:0];
            end
            default: begin
            end
        endcase
    ...
    always_comb begin
        if (read_bank_q == 2'd0) begin
            case (read_addr_q)
                2'd0: read_data = bank_a_q[0];
                2'd1: read_data = bank_a_q[1];
                2'd2: read_data = bank_a_q[2];
                default: read_data = bank_a_q[3];
            endcase
        end else if (read_bank_q == 2'd1) begin
            case (read_addr_q)
                2'd0: read_data = bank_b_q[0];
                2'd1: read_data = bank_b_q[1];
                2'd2: read_data = bank_b_q[2];
                default: read_data = bank_b_q[3];
            endcase
        end else begin
            case (read_addr_q)
                2'd0: read_data = bank_c_q[0];
                2'd1: read_data = bank_c_q[1];
                2'd2: read_data = bank_c_q[2];
                default: read_data = bank_c_q[3];
            endcase
        end
    end
        ...
        if (!rst_n) begin
            ...
            case (state_q)
                STATE_IDLE: begin
                    if (cmd_stb) begin
                        case (cmd)
                            CMD_WRITE_A: bank_a_q[addr] <= {aux_in, payload};
                            CMD_WRITE_B: bank_b_q[addr] <= {aux_in, payload};
                            CMD_READ: begin
                ...
                STATE_EXEC: begin
                    case (exec_op_q)
                        OP_EW_ADD: bank_c_q[exec_addr_q] <= bank_a_q[exec_addr_q] + bank_b_q[exec_addr_q];
